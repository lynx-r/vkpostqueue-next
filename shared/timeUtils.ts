...
export const dateAndTimeFormatter = (date: string, time: string) =>
  date + DATETIME_FMT_SPLIT + time
...
export const isValidDatetime = (date: string, time: string): boolean =>
  isValid(parseDateAndTime(date, time))
export const isValidTime = (time: string): boolean =>
  isValid(parse(time, TIME_FMT, new Date()))
...
export const docTitleToName = (title: string) =>
  title.substr(title.indexOf('_') + 1)
